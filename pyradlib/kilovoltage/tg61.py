...
def mass_absorption_coefficient(hvl : float, unit : str = 'mm Al'):
    ...
    Args:
        hvl (float): The half-value layer of the radiation.
        unit (str): The unit of the half-value layer ('mm Al' or 'mm Cu').
    """
    # pre-conditions
...
def backscatter_factor(hvl : float, unit : str = 'mm Al', diameter : float = 5.0, ssd : float = 30.0, close_ended_correction : bool = False):
    """ Determine the water-kerma based backscatter factor for a water phantom.
    
    Args:
        hvl (float): The half-value layer of the radiation.
        unit (str): The unit of the half-value layer ('mm Al' or 'mm Cu').
        diameter (float): The field diameter, in cm.
        ssd (float): The nominal focal source to surface distance, in cm.
        close_ended_correction (bool): The flag for multiplicative close-ended cone correction factor use.
    """
    # pre-conditions
    if unit not in ['mm Al', 'mm Cu']:
        raise ValueError('Invalid filter specified.')
